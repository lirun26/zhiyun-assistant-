...
## 小说创作流程（2026-04-01 新增，2026-04-12 更新集成AI-Novel-Writing-Assistant）

### ⚠️ 每次创作新章节前，必须严格按以下6步执行：

#### 第1步：读取skill指导
- 读取 `open-novel-writing` 的SKILL.md（基础写作规范）
- 读取 `ai-novel-writing-assistant` 的SKILL.md（AI辅助工具）
- ⭐ 读取风水知识库 `.learnings/FENG_SHUI_KNOWLEDGE.md`（涉及风水内容时必读）

#### 第2步：AI辅助章节规划（推荐使用AI-Novel-Writing-Assistant）
**使用AI-Novel-Writing-Assistant**：
```bash
# 检查服务状态
...
#### 第3步：创作正文
- 按spec创作，确保≥4000字
- 禁止P0红线（AI词汇/感悟式结尾/感叹式结尾/上帝视角）

#### 第4步：创建评审报告（必须执行）
| 检查项 | 内容 |
|:---|:---|
| 字数统计 | wc -m 确认≥4000 |
| P0红线 | AI词汇/感悟式/感叹式/上帝视角/时间线矛盾 |
| P1检查 | 套路化/流水账/证据不足/重复描写 |
| 逻辑检查 | 人物状态/位置/动机是否合理 |
| 评分 | 5角色评分，综合≥85分 |
...
```bash
# 快速检查（发前必跑）
echo "=== 修仙术语 ===" && grep -n "炼气\|金丹\|筑基\|灵力\|修为" 第XX章*.md || echo "无 ✅"
echo "=== 叶家检查 ===" && grep -n "叶家\|叶氏" 第XX章*.md || echo "无 ✅"
echo "=== 字数 ===" && wc -m 第XX章*.md
echo "=== 结尾 ===" && tail -3 第XX章*.md
echo "=== AI味检测 ===" && grep -c "突然\|就在这时\|不得不" 第XX章*.md
```
